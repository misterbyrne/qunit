  ...
  Test.prototype = {
  	...
  	run: function run() {
  		...
  		try {
  			...
  			this.pushFailure("Died on test #" + (this.assertions.length + 1) + " " + this.getStack() + ": " + (e.message || e), extractStacktrace(e, 0));
  	...
  	finish: function finish() {
  		...
  		if (this.steps.length) {
  			...
  			this.pushFailure("Expected assert.verifySteps() to be called before end of test " + ("after using assert.step(). Unverified steps: " + stepsList), this.getStack());
  		}

  		if (config.requireExpects && this.expected === null) {
  			this.pushFailure("Expected number of assertions to be defined, but expect() was " + "not called.", this.getStack());
  		} else if (this.expected !== null && this.expected !== this.assertions.length) {
  			this.pushFailure("Expected " + this.expected + " assertions, but " + this.assertions.length + " were run", this.getStack());
  		} else if (this.expected === null && !this.assertions.length) {
  			this.pushFailure("Expected at least one assertion, but none were run - call " + "expect(0) to accept zero assertions.", this.getStack());
  		}
